...
class WebsocketManager(object):
    ...
    def __init__(self, interval=0.1, config_dict=None, data_queue=None):
        ...
        self.processes.append(mp.Process(name="WebsocketServer", target=self.spawnWebsocket))
            ...
            async for message in websocket:
                ...
                if message == "req_connect":
                    ...
                    while True:
                        ...
                        if dbg: print("sending", inp)
                        await websocket.send(inp)
                        if inp == '\n':
                            if dbg: print("recieved empty input -> end")
                            break
